...
## 2026-03-03 A 方案交互修复（可信度与可用性）

### 任务拆解
1. 修复时间语义：来源发布时间/事件来源时间不再被轮询时间覆盖，保留原始来源时间语义。
2. 修复内容警示门禁：`ContentWarningModal` 使用全局层级渲染，避免被页面 transform 影响而可绕过。
3. 优化实时信号带：支持暂停/继续，尊重 reduced-motion，提升文本可读性（避免地名与正文粘连）。
4. 强化来源抽屉可用性：增加 backdrop、Esc 关闭、焦点管理与 Tab 限定，避免关闭态仍可误交互。
5. 更新/新增测试，覆盖上述 4 项行为。
6. 执行验证：定向测试 + 全量测试 + 构建。

### 依赖关系
- 2/3/4 依赖 1（语义与交互基线先一致）。
- 5 依赖 1/2/3/4。
- 6 依赖 5。

### 验收标准
- 页面中的来源发布时间字段保持来源语义，不会被“探测时间”直接替换。
- 内容警示弹层在未确认前不可被首屏其它控件绕过。
- 实时信号带可被手动暂停，并在系统偏好减少动画时停止自动滚动。
- Source Drawer 具备 backdrop 与键盘关闭能力，打开时焦点在抽屉内可控。
- `npm test`、`npm run build` 全部通过。
